...
use nom::{digit,alpha,alphanumeric,GetInput,ErrorKind,IResult};
use std;
use std::str;
use std::str::FromStr;
use std::io::prelude::*;
...
named!(module <Module>, do_parse!(
  ...
  name: return_error!(ErrorKind::Custom(1), sort) >>
  complete!(many0!(alt_complete!(blockcomment | comment))) >>
...
pub fn parse_string(data: &[u8]) -> Program {
  ...
  match &a {
    &IResult::Done(ref x,ref y) => {},
    &IResult::Error(ref e) => {println!("ERROR {}", e); panic!("")},
    &IResult::Incomplete(ref x) => {}
  }
