    ...
    where [I: Stream<Item=char>]
{
    ...
    skipnocode()
        ...
        .skip(eof())
        .map(
            |(e, p, m): (NamedElement, Vec<NamedProcedure>, Vec<NamedModule>)| {
                Program::new(e, m, p)
            },
        )
}
    ...
    where [I: Stream<Item=char>]
{
    char('$')
        ...
        .map(|(x, ind)| NamedElement::Dollar('$'.to_string() + &x, ind.map(Box::new)))
}
}
    ...
    where [I: Stream<Item=char>]
{
    ...
    let expand = keyword("expand")
        .skip(statementend())
        .map(|_| NamedStatement::Expand);
    let print = keyword("print")
        .skip(statementend())
        .map(|_| NamedStatement::Print);
    let maximum = keyword("maximum")
        .with(dollarvar())
        .skip(statementend())
        .map(|x| NamedStatement::Maximum(x));
    let call_procedure = (
    ...
    where [I: Stream<Item=char>]
{
    ...
    let variableargument =
        (char('?'), varname()).map(|(_, v)| NamedElement::VariableArgument("?".to_owned() + &v));
    ...
    let namedfactor = varname()
        .and(choice!(
            ...
            value(1).map(|_| NamedElement::Var(String::new()))
        ))
        .map(|(name, mut res)| {
            match res {
                NamedElement::Wildcard(ref mut n, ..) | NamedElement::Var(ref mut n) => *n = name,
